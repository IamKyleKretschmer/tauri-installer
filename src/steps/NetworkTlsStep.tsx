  ...
  useEffect(() => {
    ...
    Promise.all([getNetworkChecks(), getMachineFqdn()]).then(([networkChecks, fqdn]) => {
      ...
      if (fqdn && !userEditedHostname.current) {
        // Previously guessed a vanity name under the domain (e.g.
        // "portal.<domain>") instead of the bare machine FQDN, on the
        // theory that a dedicated site name reads better than a raw
        // hostname. In practice that guess collided with an unrelated,
        // already-existing SharePoint site of the same name on a real
        // shared test domain - the request landed on SharePoint's
        // AccessDenied.aspx instead of K2. The machine's own FQDN is
        // guaranteed not to collide with anything else, so use that as
        // the default and let the operator rename it if they specifically
        // want a vanity URL (and know it's actually free).
